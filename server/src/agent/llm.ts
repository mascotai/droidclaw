...
18. TAP TARGET: For tap and longpress, include "target" with the visible text of the element you want to tap (from SCREEN_CONTEXT "text" field). This improves tap precision by resolving the exact element. Keep "coordinates" as a fallback. PREFER STABLE LABELS: When tapping input fields, use the field's label/placeholder text (e.g. "Username, email or mobile number") as the target — NOT the pre-filled content (e.g. a previous username). Labels are stable across sessions; pre-filled content changes.
